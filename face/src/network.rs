...
pub fn download_img(token: &str) -> String {
	let url = format!("https://hackattic.com/challenges/basic_face_detection/problem?access_token={token}");
...
pub struct ApiSolve {
	face_tiles: Vec<[usize; 2]>,
}

pub fn send_secret(face_tiles: Vec<[usize; 2]>, token: &str) {
	let url = format!("https://hackattic.com/challenges/basic_face_detection/solve?access_token={token}");
	let mut res = ureq::post(url).send_json(&ApiSolve { face_tiles }).unwrap();
